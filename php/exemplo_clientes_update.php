...
<body>
    ...
    <div>
        ...
        <button type="button"><a href='./exemplo_clientes.php'>CADASTRAR</a></button>
        <div>

            <h2>Inserir</h2>
            <form action="exemplo_clientes_update.php" method="post">
                ...
                NOVO_NOME: <input type="text" name="novo_nome" id="">
                <input type="submit">
            </form>
        ...
        <div>
            ...
            if ($nome != '') {
                $consulta_sql = <<<HEREDOC
                    UPDATE clientes SET nome = ?, email = ?, idade = ?, cidade = ?, saldo = ?, telefone = ? WHERE nome = ?;
                HEREDOC;
                $matriz_tabela = array();
                $resultados = $conn->prepare($consulta_sql);
                $resultados->execute(array($novo_nome, $email, $idade, $cidade, $saldo, $telefone, $nome));
            }
